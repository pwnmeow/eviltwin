...
class MyPortal extends Portal {
    ...
    public function handleAuthorization() {
        ...
        // Check if username is set and not empty, else default to 'Alfa'
        if (isset($this->request->username) && strlen(trim($this->request->username)) > 0) {
            $this->request->username = trim($this->request->username);
        } else {
            $this->request->username = 'Alfa'; // Default username
        }

        if (strlen($this->request->username) <= $limit) {
